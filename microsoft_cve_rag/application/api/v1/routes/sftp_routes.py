...
async def get_sftp_settings() -> SFTPSettings:
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"SFTP configuration error: {e}"
        )
    except Exception as e:
        logger.error(
            f"Unexpected error loading SFTP settings: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail="Failed to load SFTP settings"
        )
...
async def check_sftp_status(
    ...
    try:
        ...
        service = SFTPService(
            settings
        )  # noqa: F841 (unused variable is ok here)
        logger.info(
            "SFTPService instantiated successfully (settings"
            f" loaded).\nAsType: {service}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error instantiating SFTPService: {e}"
        )
...
class RemotePathQuery(BaseModel):
    remote_path: str = Field(
        default=".",
        description=(
            "Remote path on the SFTP server (e.g., '.', '/public_html',"
            " 'reports/subdir')"
        ),
    )
...
async def list_remote_directory(
    ...
    """
    logger.info(
        f"Route /sftp-test/list called for path: {path_query.remote_path}"
    )
    service = SFTPService(settings)
    try:
        with (
            service
        ):  # Handles connect/disconnect and initial chdir if implemented
            logger.info(
                f"Attempting to list directory: {path_query.remote_path}"
            )
            contents = service.list_directory(path_query.remote_path)
            logger.info(
                f"Successfully listed directory '{path_query.remote_path}'."
                f" Found {len(contents)} items."
            )
            return {
                "remote_path": path_query.remote_path,
                "current_directory_after_list": (
                    service.get_current_directory()
                ),  # Get CWD after operation
                "contents": contents,
            }
    except (ConnectionError, AuthenticationException, SSHException) as e:
        logger.error(
            "SFTP Connection/Auth error listing"
            f" {path_query.remote_path}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=503, detail=f"SFTP Connection/Auth error: {e}"
        )
    except (OSError, IOError) as e:
        logger.error(
            f"SFTP OS/IO error listing {path_query.remote_path}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=404,
            detail=(
                f"SFTP Error listing directory '{path_query.remote_path}': {e}"
            ),
        )  # 404 if path likely not found
    except Exception as e:
        logger.exception(
            f"Unexpected error listing {path_query.remote_path}: {e}"
        )
        raise HTTPException(
            status_code=500, detail=f"Unexpected SFTP error: {e}"
        )
...
async def change_remote_directory(
    ...
    """
    logger.info(
        f"Route /sftp-test/chdir called for path: {path_query.remote_path}"
    )
    if not path_query.remote_path:
        raise HTTPException(
            status_code=400, detail="Parameter 'remote_path' cannot be empty."
        )
    ...
    try:
        with service:
            logger.info(
                f"Attempting to change directory to: {path_query.remote_path}"
            )
        ...
        logger.error(
            "SFTP Connection/Auth error changing dir to"
            f" {path_query.remote_path}: {e}",
            exc_info=True,
        )
        raise HTTPException(
        ...
        logger.error(
            f"SFTP OS/IO error changing dir to {path_query.remote_path}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=404,
            detail=(
                "SFTP Error changing directory to"
                f" '{path_query.remote_path}': {e}"
            ),
        )  # 404 if path likely not found
    except Exception as e:
        logger.exception(
            f"Unexpected error changing dir to {path_query.remote_path}: {e}"
        )
        raise HTTPException(
            status_code=500, detail=f"Unexpected SFTP error: {e}"
        )
...
async def upload_test_files(
    ...
    if not REPORTS_DIR or not REPORTS_DIR.is_dir():
        logger.error(
            "REPORTS_DIR global path is not set or directory not found:"
            f" {REPORTS_DIR}"
        )
        raise HTTPException(
            status_code=500,
            detail="Server configuration error: REPORTS_DIR not available.",
        )
    ...
    try:
        ...
        if not local_html_path.exists():
            ...
            local_html_path.write_text(
                "<html><body><h1>SFTP Test Report</h1></body></html>",
                encoding='utf-8',
            )
        if not local_thumb_path.exists():
            logger.warning(f"Creating dummy local file: {local_thumb_path}")
            local_thumb_path.write_text(
                "dummy png data", encoding='utf-8'
            )  # Simple text for dummy png
    except OSError as e:
        logger.error(
            f"Failed to create local test files in {local_test_dir}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500, detail=f"Failed to prepare local test files: {e}"
        )

    # --- Define Remote Test Paths ---
    remote_base_dir = (  # Choose a safe test directory name
        "www/portalfuse.io/public_html/wp-content/uploads/kb_weekly"
    )
    remote_html_path = (
        f"{remote_base_dir}/html/report_{Path(local_html_path.name).stem}.html"
    )
    ...
    try:
        with service:
            ...
            try:
                logger.info(
                    f"Attempting to upload {local_html_path} to"
                    f" {remote_html_path}"
                )
                ...
                FileNotFoundError
            ):  # Should not happen if creation above worked
                logger.error(
                    "Local file missing during upload attempt:"
                    f" {local_html_path}"
                )
                errors.append(f"Local file missing: {local_html_path}")
            except (OSError, IOError, ConnectionError, SSHException) as e:
                logger.error(
                    f"Failed to upload {local_html_path}: {e}", exc_info=True
                )
            ...
            try:
                logger.info(
                    f"Attempting to upload {local_thumb_path} to"
                    f" {remote_thumb_path}"
                )
                ...
                logger.error(
                    f"Failed to upload {local_thumb_path}: {e}", exc_info=True
                )
                errors.append(f"Failed uploading {local_thumb_path}: {e}")

    except (ConnectionError, AuthenticationException, SSHException) as e:
        logger.error(
            f"SFTP Connection/Auth error during upload test: {e}",
            exc_info=True,
        )
...
@router.get(
    "/validate-kb-weekly",
    summary="Validate 'kb_weekly' directory structure and file counts",
)
async def validate_kb_weekly_structure(
    # Optional: Allow overriding the base path if needed for testing/other reports
    base_path: str = Query(
        "kb_weekly",
        description="The base directory to validate (defaults to 'kb_weekly')",
    ),
    settings: SFTPSettings = Depends(get_sftp_settings),
):
    """
    ...
    """
    logger.info(
        "Route /sftp-test/validate-kb-weekly called for base_path:"
        f" {base_path}"
    )
    ...
    try:
        with service:  # Handles connect/disconnect
            sftp = (
                service._sftp_client
            )  # Get direct access for stat/listdir_attr
            ...
            try:
                ...
                if stat.S_ISDIR(base_attrs.st_mode):
                    ...
                    validation_results["details"].append(
                        f"Base path '{base_path}' exists and is a directory."
                    )
                    validation_results["overall_status"] = "OK"  # Tentative OK
                else:
                    validation_results["details"].append(
                        f"Error: Base path '{base_path}' exists but is not a"
                        " directory."
                    )
                ...
                validation_results["details"].append(
                    f"Error: Base path '{base_path}' not found."
                )
                validation_results["overall_status"] = "Error"
                return validation_results  # Cannot proceed further
            except (OSError, IOError) as e:
                logger.error(
                    f"Error checking base path '{base_path}': {e}",
                    exc_info=True,
                )
                validation_results["details"].append(
                    f"Error checking base path '{base_path}': {e}"
            ...
            for subdir_name, expected_data in validation_results[
                "subdirectories"
            ].items():
                current_subdir_path = (  # Use forward slash for remote paths
                    f"{base_path}/{subdir_name}"
                )
                ...
                try:
                    ...
                    if stat.S_ISDIR(subdir_attrs.st_mode):
                        ...
                        validation_results["details"].append(
                            f"Subdirectory '{current_subdir_path}' exists and"
                            " is a directory."
                        )
                        ...
                        try:
                            logger.debug(
                                "Listing attributes for:"
                                f" {current_subdir_path}"
                            )
                            # Use listdir_attr to get file types
                            contents_attrs = sftp.listdir_attr(
                                current_subdir_path
                            )
                            file_count = 0
                            ...
                            if file_count == expected_data["expected_files"]:
                                validation_results["details"].append(
                                    f"Correct file count ({file_count}) found"
                                    f" in '{current_subdir_path}'."
                                )
                            else:
                                validation_results["details"].append(
                                    "Mismatch: Expected"
                                    f" {expected_data['expected_files']} files"
                                    f" but found {file_count} in"
                                    f" '{current_subdir_path}'."
                                )
                                validation_results["overall_status"] = (
                                    "Mismatch"  # Mark overall as mismatch
                                )

                        except (OSError, IOError) as list_e:
                            logger.error(
                                "Error listing contents of"
                                f" '{current_subdir_path}': {list_e}",
                                exc_info=True,
                            )
                            validation_results["details"].append(
                                "Error listing contents of"
                            ...
                            validation_results["overall_status"] = (
                                "Error"  # Listing error is more severe
                            )

                    else:  # Exists but is not a directory
                        expected_data["is_dir"] = False
                        validation_results["details"].append(
                            f"Error: Expected '{current_subdir_path}' to be a"
                    ...
                    logger.warning(
                        f"Subdirectory '{current_subdir_path}' not found."
                    )
                    expected_data["exists"] = False
                    validation_results["details"].append(
                        f"Error: Expected subdirectory '{current_subdir_path}'"
                        " not found."
                    )
                    validation_results["overall_status"] = (
                        "Mismatch"  # Missing directory is a mismatch
                    )
                except (OSError, IOError) as stat_e:
                    logger.error(
                        f"Error checking subdirectory '{current_subdir_path}':"
                        f" {stat_e}",
                        exc_info=True,
                    )
                    validation_results["details"].append(
                    ...
                    validation_results["overall_status"] = (
                        "Error"  # Treat stat errors as more severe
                    )
            # Final check on status
            if validation_results["overall_status"] == "OK":
                logger.info(
                    f"Validation successful for base path '{base_path}'."
                )
            else:
                logger.warning(
                    f"Validation found issues for base path '{base_path}'."
                    f" Status: {validation_results['overall_status']}"
                )
        ...
        logger.error(
            "SFTP Connection/Auth error during validation of"
            f" {base_path}: {e}",
            exc_info=True,
        )
        raise HTTPException(
        ...
        logger.exception(
            f"Unexpected error during validation of {base_path}: {e}"
        )
        raise HTTPException(
            status_code=500,
            detail=f"Unexpected SFTP error during validation: {e}",
        )
