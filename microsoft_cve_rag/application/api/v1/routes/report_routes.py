...
class QuarterlyReportRequest(BaseModel):
    ...
    start_date: datetime
    end_date: datetime
    config_override: Optional[DeepDiveReportConfig] = Field(
        None,
        description="Optional overrides for default report configuration.",
    )
...
async def get_document_service() -> DocumentService:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="DocumentService configuration error."
        )
    except Exception as e:
        logger.error(f"ERROR: Failed to instantiate DocumentService: {e}")
        raise HTTPException(
            status_code=500, detail=f"DocumentService instantiation error: {e}"
        )
...
def get_llm_client() -> LLMClient:
    ...
    try:
        ...
        mock_mode = mock_mode_str.lower() in (
            'true',
            '1',
            't',
            'yes',
        ...
        logger.exception(
            f"Failed to instantiate LLMClient: {e}"
        )  # Use logger.exception for traceback
        raise HTTPException(
            status_code=500, detail="LLM Client configuration error"
        )
...
async def get_jinja_environment() -> JinjaEnvironment:
    ...
    if not template_dir.is_dir():
        ...
        raise HTTPException(
            status_code=500,
            detail="Server configuration error: Templates directory missing.",
        )

    logging.debug(
        f"Initializing Jinja2 environment with loader path: {template_dir}"
    )
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Server configuration error: Jinja initialization failed.",
        )
...
def get_report_generator(
    ...
    try:
        ...
        if REPORTS_DIR is None or not REPORTS_DIR.is_dir():
            logging.error(
                "REPORTS_DIR is not initialized or not a directory after env"
                f" init: {REPORTS_DIR}"
            )
            raise HTTPException(
                status_code=500,
                detail=(
                    "Server configuration error: Reports directory not"
                    " available."
                ),
            )
        if DATA_DIR is None or not DATA_DIR.is_dir():
            logging.error(
                "DATA_DIR is not initialized or not a directory after env"
                f" init: {DATA_DIR}"
            )
            raise HTTPException(
                status_code=500,
                detail=(
                    "Server configuration error: Data directory not available."
                ),
            )
        if APP_DIR is None or not APP_DIR.is_dir():
            logging.error(
                "APP_DIR is not initialized or not a directory after env"
                f" init: {APP_DIR}"
            )
            raise HTTPException(
                status_code=500,
                detail=(
                    "Server configuration error: App directory not available."
                ),
            )
        ...
        if not templates_base_dir.is_dir():
            logging.error(
                "Templates base directory not found at expected location:"
                f" {templates_base_dir}"
            )
            raise HTTPException(
                status_code=500,
                detail=(
                    "Server configuration error: Templates directory not"
                    " found."
                ),
            )
        ...
        logger.info(
            "Type of jinja_env received in get_report_generator:"
            f" {type(jinja_env)}"
        )
        if not isinstance(jinja_env, JinjaEnvironment):
            logger.error(
                "Incorrect type received for jinja_env dependency:"
                f" {type(jinja_env)}. Expected JinjaEnvironment."
            )
            # This indicates a deeper issue in FastAPI setup or the dependency function itself
            raise HTTPException(
                status_code=500,
                detail="Internal server error: Invalid Jinja configuration.",
            )
        # --- End Debugging ---
        ...
        logging.exception(
            "Fatal error during environment initialization within dependency."
        )
        raise HTTPException(
        ...
        raise HTTPException(
            status_code=500, detail=f"Could not create report generator: {e}"
        )
...
def get_metrics_service() -> DuckDBMetricsService:
    ...
    """
    logger.info(
        "Attempting to create/retrieve cached DuckDBMetricsService instance..."
    )
    try:
        ...
        logger.error(
            f"Failed to get metrics credentials for service instantiation: {e}"
        )
        raise HTTPException(
...
async def generate_kb_report(
    ...
    try:
        ...
        kb_service = DocumentService(
            db_name="report_docstore", collection_name="microsoft_kb_articles"
        )
        docstore_service = DocumentService(
            db_name="report_docstore", collection_name="docstore"
        )
        ...
        cve_details = extract_cve_details_for_report(
            list(cve_ids), list(kb_ids), docstore_service
        )

        # Transform data
        cve_lookup = process_cve_data_for_kb_report(cve_details)
        kb_data = await transform_kb_data_for_kb_report(
            kb_articles, cve_lookup
        )

        # Convert DataFrame to records and handle JSON serialization
        kb_records = kb_data.replace(
        ...
        if kb_records:
            ...
            logging.info(
                "os_classification:"
                f" {sample.get('os_classification', 'NOT FOUND')}"
            )
            logging.info(f"cve_details: {sample.get('cve_details', {})}")
        ...
        try:
            ...
            if hasattr(e, '__traceback__'):
                ...
                if kb_records:
                    ...
                    logging.error(
                        "cve_details structure:"
                        f" {json.dumps(sample.get('cve_details', {}), indent=2)}"
                    )
        ...
        if not request.end_date or not isinstance(request.end_date, datetime):
            if isinstance(request.end_date, str):
                report_date = datetime.strptime(
                    request.end_date, report_date_format
                )
            else:
                report_date = datetime.now()
                logging.warning(
                    "Report date not provided or invalid, using current date."
                )
        else:
            report_date = request.end_date
        # begin sftp workflow
        ...
        browser_cfg = BrowserConfig(
            headless=False, viewport_width=1140, viewport_height=1280
        )
        ...
        async def _run_crawl_async_task():
            ...
            async with AsyncWebCrawler(config=browser_cfg) as crawler:
                return await crawler.arun(
                    url=f"{base_url}/html/{output_file_name}", config=run_cfg
                )
        ...
        def _run_crawl_sync_wrapper():
            ...
            try:
                ...
                logging.error(
                    f"Error inside _run_crawl_sync_wrapper: {e}", exc_info=True
                )
                # Re-raise or return an error indicator if needed
                raise  # Re-raise to be caught by the main await

        loop = asyncio.get_running_loop()
        crawl_result: CrawlResult = await loop.run_in_executor(
            None, _run_crawl_sync_wrapper
        )
        ...
        if crawl_result.markdown:
            markdown_output_filename = (
                f"kb_report_{request.end_date.strftime('%Y%m%d')}.md"
            )
            markdown_output_path = (
                report_markdown_path / markdown_output_filename
            )
        ...
        if crawl_result.screenshot:
            ...
            if image_bytes:
                try:
                    ...
                    crop_height = math.ceil(
                        min(
                            ideal_crop_height, MAX_CROP_HEIGHT, original_height
                        )
                    )
                    ...
                    lower = (
                        upper + crop_height
                    )  # Height already constrained by original_height
                    ...
                    screenshot_local_path = (
                        report_thumbnail_path / screenshot_output_filename
                    )
                    ...
                    FileNotFoundError
                ):  # From Image.open if stream is invalid? Unlikely but possible
                    logger.error("Error: Could not process image stream.")
                except Exception as e:
                    logger.error(
                        f"An error occurred processing the image: {e}"
                    )
            with sftp_service:
                sftp_service.upload_file(
        ...
        return {
            ...
            "message": (
                f"Successfully processed {len(kb_articles)} KB articles"
            ),
        }

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to generate KB report: {str(e)}"
        )
...
async def generate_quarterly_report_prod(
    ...
    """
    logger.info(
        f"Received request for production report: {request.start_date} to"
        f" {request.end_date}"
    )
    import traceback
    ...
    try:
        ...
        if not report_data_list:
            logger.warning(
                "Warning: No documents found in MongoDB for the specified date"
                " range."
            )
    ...
    try:
        ...
        logger.info(
            ...
            f" {generated_assets.base_directory}"
        )
    except Exception as e:
    ...
    return {
        ...
        "generated_files": [
            str(f) for f in generated_assets.get_all_files() if f
        ],
    }
...
@router.post(
    "/reports/quarterly-deep-dive/generate-dev-synthetic",
    tags=["Reports Development"],
)
async def generate_quarterly_report_dev_synthetic(
    ...
    """
    logger.info(
        f"Received request for SYNTHETIC report: {request.start_date} to"
        f" {request.end_date}"
    )
    import traceback
    ...
    try:
        ...
        loop = asyncio.get_running_loop()
        synthetic_df = await loop.run_in_executor(
            None,
            generate_synthetic_cve_data,
            request.start_date,
            request.end_date,
    ...
    try:
        ...
        logger.info(
            "Synthetic report generation complete. Assets in:"
            f" {generated_assets.base_directory}"
        )
    except Exception as e:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Synthetic report generation failed: {str(e)}",
        )
