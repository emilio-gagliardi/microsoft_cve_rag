...
try:
    ...
    logging.error(
        f"Failed to import necessary modules for Azure Blob router: {e}"
    )
    raise
...
@router.get(
    "/status", summary="Check Azure Blob Storage configuration loading"
)
async def check_blob_status(
    ...
    try:
        ...
        logger.info(
            "AzureBlobStorageService instantiated successfully (account"
            " settings loaded)."
        )
        ...
        try:
            ...
            init_status = (
                "BlobServiceClient initialized successfully (connection likely"
                " OK)."
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error instantiating Service: {e}"
        )
...
async def upload_test_blob(
    ...
    """
    logger.info(
        "Route /blob-storage-test/upload-test called for container:"
        f" '{container_name}'."
    )
    ...
    if not REPORTS_DIR or not REPORTS_DIR.is_dir():  # Check REPORTS_DIR
        ...
        raise HTTPException(
            status_code=500,
            detail="Server configuration error: Base directory not available.",
        )
    ...
    try:
        logger.info(
            f"Attempting to upload '{local_file_path}' to blob '{blob_name}'"
        ...
        logger.error(
            f"Failed to upload test blob '{blob_name}' to container"
            f" '{container_name}': {e}",
            exc_info=True,
        )
        status_code = 503 if isinstance(e, ConnectionError) else 500
        raise HTTPException(
            status_code=status_code, detail=f"Failed to upload to Azure: {e}"
        )
    except Exception as e:
        logger.exception(f"Unexpected error during blob upload test: {e}")
        raise HTTPException(
            status_code=500, detail=f"Unexpected error during upload: {e}"
        )
...
class DeleteBlobRequest(BaseModel):
    ...
    blob_name: str = Field(
        ...,
        description=(
            "Full name/path of the blob to delete within the container."
        ),
    )
...
@router.post(
    "/delete-blob",
    summary="Delete a specific blob",
    response_model=DeleteBlobResponse,
)
async def delete_test_blob(
    ...
    logger.info(
        f"Route /blob-storage-test/delete-blob called for blob: '{blob}' in"
        f" container '{container}'"
    )
    if not container or not blob:
        raise HTTPException(
            status_code=400,
            detail="container_name and blob_name are required.",
        )
    ...
    try:
        ...
        message = (
            f"Blob '{blob}' deleted successfully from '{container}'."
            if deleted
            else f"Blob '{blob}' not found in container '{container}'."
        )
        logger.info(message)
        ...
        logger.error(
            f"Failed to delete blob '{blob}' from container"
            f" '{container}': {e}",
            exc_info=True,
        )
        status_code = 503 if isinstance(e, ConnectionError) else 500
        raise HTTPException(
            status_code=status_code, detail=f"Failed to delete blob: {e}"
        )
    except Exception as e:
        logger.exception(
            f"Unexpected error during blob deletion for '{blob}' in"
            f" '{container}': {e}"
        )
        raise HTTPException(
            status_code=500, detail=f"Unexpected error during deletion: {e}"
        )
...
class DeletePrefixRequest(BaseModel):
    container_name: str = Field(
        ..., description="Name of the container to delete from."
    )
    prefix: str = Field(
        ...,
        description=(
            "Prefix (simulated folder path) to delete blobs from, e.g.,"
            " 'test_uploads/'."
        ),
    )
...
@router.post(
    "/delete-prefix",
    summary="Delete all blobs starting with a prefix",
    response_model=DeletePrefixResponse,
)
async def delete_test_prefix(
    ...
    logger.info(
        f"Route /blob-storage-test/delete-prefix called for prefix: '{prefix}'"
        f" in container '{container}'"
    )
    if not container or not prefix:
        raise HTTPException(
            status_code=400, detail="container_name and prefix are required."
        )
    ...
    try:
        ...
        message = (
            f"Successfully submitted deletion for {deleted_count} blobs with"
            f" prefix '{prefix}' in container '{container}'."
            if deleted_count > 0
            else (
        ...
        logger.error(
            f"Container not found during prefix deletion: {e}", exc_info=True
        )
        raise HTTPException(status_code=404, detail=str(e))
    except (ConnectionError, IOError, AzureError) as e:
        logger.error(
            f"Failed to delete blobs with prefix '{prefix}' in container"
            f" '{container}': {e}",
            exc_info=True,
        )
        status_code = 503 if isinstance(e, ConnectionError) else 500
        raise HTTPException(
            ...
            detail=f"Failed to delete blobs by prefix: {e}",
        )
    except Exception as e:
        logger.exception(
            f"Unexpected error during prefix deletion for '{prefix}' in"
            f" container '{container}': {e}"
        )
        raise HTTPException(
            status_code=500,
            detail=f"Unexpected error during prefix deletion: {e}",
        )
