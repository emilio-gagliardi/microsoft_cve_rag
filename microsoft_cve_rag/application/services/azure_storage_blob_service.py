...
class AzureBlobStorageService:
    ...
    def __init__(self, settings: AzureStorageSettings):
        ...
        logger.info(
            "AzureBlobStorageService initialized for account"
            f" '{settings.account_name}'. Ready to connect."
        )
    ...
    def __enter__(self) -> 'AzureBlobStorageService':
        ...
        if not self.is_connected:
            try:
                logger.info(
                    "Connecting to Azure Blob Storage account"
                    f" '{self._settings.account_name}'..."
                )
                # Use get_secret_value() for secure key handling if using Pydantic SecretStr etc.
                credential = (
                    self._settings.account_key.get_secret_value()
                    if hasattr(self._settings.account_key, 'get_secret_value')
                    else self._settings.account_key
                )
                ...
                self._blob_service_client = (
                    None  # Ensure client is None on failure
                )
                ...
                logger.error(
                    f"Unexpected error connecting to Azure Blob Storage: {e}",
                    exc_info=True,
                )
                self._blob_service_client = (
                    None  # Ensure client is None on failure
                ...
                raise ConnectionError(
                    f"Unexpected error connecting to Azure: {e}"
                ) from e
        return self
    ...
    def __exit__(
        ...
        if self._blob_service_client:
            try:
                ...
                logger.error(
                    f"Error closing Azure BlobServiceClient: {e}",
                    exc_info=True,
                )
            finally:
    ...
    def _get_container_client(self, container_name: str) -> ContainerClient:
        ...
        assert (
            self._blob_service_client is not None
        ), "Connection not established before getting container client"
        try:
            ...
            logger.error(
                "Failed to get Azure ContainerClient for"
                f" '{container_name}': {e}",
                exc_info=True,
            )
            ...
            logger.error(
                "Unexpected error getting Azure ContainerClient for"
                f" '{container_name}': {e}",
                exc_info=True,
            )
            raise IOError(
    ...
    def upload_blob(
        ...
        logger.info(
            f"Attempting to upload data to blob: '{blob_name}' in container"
            f" '{container_name}'"
        )
        ...
        if isinstance(data, Path):
            ...
            try:
                ...
                logger.error(
                    f"Error reading local file {data}: {e}", exc_info=True
                )
            ...
            raise TypeError(
                "Unsupported data type for upload. Use bytes, str, or"
                " pathlib.Path."
            )
        ...
        try:
            ...
            logger.info(
                f"Uploading {len(processed_data)} bytes to '{blob_name}' in"
                f" '{container_name}'. Overwrite={overwrite}"
            )
            client.upload_blob(
                name=blob_name, data=processed_data, overwrite=overwrite
            )
            logger.info(
                f"Successfully uploaded to blob: '{blob_name}' in container"
                f" '{container_name}'"
            )
        except ResourceNotFoundError:
            # This error usually means the *container* doesn't exist when uploading
            logger.error(
                f"Container '{container_name}' not found during upload to blob"
                f" '{blob_name}'."
            )
            raise FileNotFoundError(
                f"Azure container '{container_name}' not found."
            )
        except AzureError as e:
            logger.error(
                f"Azure storage error uploading blob '{blob_name}' to"
                f" container '{container_name}': {e}",
                exc_info=True,
            )
            if (
                hasattr(e, 'error_code')
                and e.error_code == StorageErrorCode.authentication_failed
            ):
            ...
            logger.error(
                f"Unexpected error uploading blob '{blob_name}' to container"
                f" '{container_name}': {e}",
                exc_info=True,
            )
            raise IOError(
    ...
    def delete_blob(self, container_name: str, blob_name: str) -> bool:
        ...
        self._ensure_connection()
        logger.info(
            f"Attempting to delete blob: '{blob_name}' from container"
            f" '{container_name}'"
        )
        try:
            client = self._get_container_client(container_name)
            ...
            logger.info(
                f"Successfully deleted blob: '{blob_name}' from container"
                f" '{container_name}'"
            )
            return True
        except ResourceNotFoundError:
            ...
            logger.warning(
                f"Blob '{blob_name}' or container '{container_name}' not"
                " found. Cannot delete."
            )
            return False  # Treat as "did not delete because it wasn't there"
        except AzureError as e:
            logger.error(
                f"Azure storage error deleting blob '{blob_name}' from"
                f" container '{container_name}': {e}",
                exc_info=True,
            )
            raise IOError(
            ...
            logger.error(
                f"Unexpected error deleting blob '{blob_name}' from container"
                f" '{container_name}': {e}",
                exc_info=True,
            )
            raise IOError(
    ...
    def delete_blobs_with_prefix(
        ...
        if not prefix:
            logger.warning(
                "Attempted to delete blobs with an empty prefix in container"
                " '%s'. No action taken.",
                container_name,
            )
            return 0
        ...
        logger.info(
            f"Attempting to delete all blobs with prefix: '{prefix}' in"
        ...
        try:
            client = self._get_container_client(container_name)
            logger.debug(
                f"Listing blobs with prefix '{prefix}' in container"
                f" '{container_name}'..."
            )
            ...
            if not blobs_to_delete:
                logger.info(
                    f"No blobs found with prefix '{prefix}' in container"
                    f" '{container_name}'. Nothing to delete."
                )
                return 0

            logger.info(
                f"Found {len(blobs_to_delete)} blobs to delete with prefix"
                f" '{prefix}' in container '{container_name}'. Submitting"
                " batch deletion..."
            )
            ...
            logger.info(
                "Successfully submitted batch deletion for"
                f" {deleted_count} blobs with prefix '{prefix}' in container"
                f" '{container_name}'."
            )
            ...
            logger.error(
                f"Container '{container_name}' not found during prefix"
                f" deletion for '{prefix}'."
            )
            raise FileNotFoundError(
                f"Azure container '{container_name}' not found."
            )
            ...
            logger.error(
                f"Azure storage error during prefix deletion for '{prefix}' in"
                f" container '{container_name}': {e}",
                exc_info=True,
            )
            raise IOError(
            ...
            logger.error(
                f"Unexpected error during prefix deletion for '{prefix}' in"
                f" container '{container_name}': {e}",
                exc_info=True,
            )
            raise IOError(
    ...
    def list_container(self, container_name: str) -> List[str]:
        ...
        try:
            ...
            logger.info(
                f"Found {len(blob_names)} blobs in container"
                f" '{container_name}'."
            )
            return blob_names
        except ResourceNotFoundError:
            logger.error(
                f"Container '{container_name}' not found during listing."
            )
            raise FileNotFoundError(
                f"Azure container '{container_name}' not found."
            )
            ...
            logger.error(
                "Azure storage error listing container"
                f" '{container_name}': {e}",
                exc_info=True,
            )
            raise IOError(
            ...
            logger.error(
                f"Unexpected error listing container '{container_name}': {e}",
                exc_info=True,
            )
            raise IOError(
                f"Unexpected error listing container '{container_name}': {e}"
    ...
    def blob_exists(self, container_name: str, blob_name: str) -> bool:
        ...
        logger.debug(
            f"Checking existence of blob '{blob_name}' in container"
            f" '{container_name}'"
        )
        try:
            ...
            logger.debug(
                f"Blob '{blob_name}' in container '{container_name}' exists:"
                f" {exists}"
            )
            return exists
        except ResourceNotFoundError:
            ...
            logger.warning(
                f"Container '{container_name}' not found while checking for"
                f" blob '{blob_name}'."
            )
            return False
        except AzureError as e:
            ...
            logger.error(
                f"Azure storage error checking existence of blob '{blob_name}'"
                f" in container '{container_name}': {e}",
                exc_info=True,
            )
            raise IOError(
                f"Azure storage error checking blob existence: {e}"
            ) from e
        except Exception as e:
            logger.error(
                f"Unexpected error checking existence of blob '{blob_name}' in"
                f" container '{container_name}': {e}",
                exc_info=True,
            )
            raise IOError(
                f"Unexpected error checking blob existence: {e}"
            ) from e
...
if __name__ == "__main__":
    ...
    try:
        storage_settings = AzureStorageSettings(
            ...
            ],  # e.g., "https://<your_account_name>.blob.core.windows.net"
            account_key=os.environ["AZURE_STORAGE_ACCOUNT_KEY"],
        )
    except KeyError:
        print(
            "Please set AZURE_STORAGE_ACCOUNT_NAME, AZURE_STORAGE_ACCOUNT_URL,"
        ...
        storage_settings = AzureStorageSettings(
            "dummyaccount",
            "https://dummyaccount.blob.core.windows.net",
            "dummykey",
        )
    ...
    try:
        ...
        with AzureBlobStorageService(storage_settings) as service:
            ...
            try:
                service.upload_blob(
                    container_name,
                    test_blob_name,
                    "Hello Azure from Context Manager!",
                )
            ...
            exists_non = service.blob_exists(
                container_name, "nonexistent_blob.dat"
            )
            ...
            try:
                deleted_count = service.delete_blobs_with_prefix(
                    container_name, test_prefix
                )
                print(
                    f"Deleted {deleted_count} blobs with prefix"
                    f" '{test_prefix}'."
                )
            except (FileNotFoundError, IOError) as e:
        ...
        logger.error(
            "An unexpected error occurred in the main block.", exc_info=True
        )
...
async def get_azure_settings() -> AzureStorageSettings:
    ...
    try:
        ...
        logger.error(
            f"Azure Storage settings validation failed: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail=f"Azure configuration error: {e}"
        )
    except Exception as e:
        logger.error(
            f"Unexpected error loading Azure settings: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail="Failed to load Azure settings"
        )
