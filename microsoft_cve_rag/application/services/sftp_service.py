...
class SFTPSettings(BaseSettings):
    ...
    port: int = Field(
        22, validation_alias='SITEGROUND_SSH_PORT'
    ...
    password: Optional[SecretStr] = Field(
        None, validation_alias='SITEGROUND_SSH_PASSWORD'
    )
...
class SFTPService:
    ...
    def __init__(self, settings: SFTPSettings):
        ...
        logger.info(
            "SFTPService initialized for"
            f" {settings.username}@{settings.hostname}:{settings.port}"
        )
    ...
    def _connect(self) -> None:
            ...
            if not self._sftp_client or self._sftp_client.sock.closed:
                ...
                try:
                    ...
                    raise ConnectionError(
                        f"Failed to reopen SFTP session: {e}"
                    ) from e
            return  # Already connected

        logger.debug(
            "Attempting to connect to"
            f" {self._settings.hostname}:{self._settings.port}..."
        )
        ...
        try:
            ...
            private_key = RSAKey.from_private_key_file(
                str(self._settings.key_path), password=key_password
            )
            ...
            logger.error(
                f"Private key file not found at {self._settings.key_path}"
            )
            raise ConnectionError(
                f"Private key file not found: {self._settings.key_path}"
            )
        except AuthenticationException as e:
            logger.error(
                f"Authentication failed for user {self._settings.username}:"
                f" {e}. Check username, key, and key password."
            )
            # Avoid logging password detail directly from settings
            logger.debug(
                f"Attempted with key: {self._settings.key_path}, password"
                f" provided: {bool(self._settings.password)}"
            )
            raise ConnectionError(f"Authentication failed: {e}") from e
        except NoValidConnectionsError as e:
            logger.error(
                f"Failed to connect to host {self._settings.hostname} on port"
                f" {self._settings.port}: {e}"
            )
            raise ConnectionError(
            ...
            if "Error reading SSH protocol banner" in str(e):
                logger.error(
                    "Could be a non-SSH service running on the port or network"
                    " issue."
                )
            elif (
                "Private key file is encrypted" in str(e) and not key_password
            ):
                logger.error(
    ...
    def _close(self) -> None:
        ...
        if self._sftp_client:
            try:
                ...
                logger.warning(
                    f"Error closing SFTP session: {e}", exc_info=True
                )
        if self._ssh_client:
            try:
                ...
                logger.warning(
                    f"Error closing SSH connection: {e}", exc_info=True
                )
    ...
    def __exit__(
        ...
        if exc_value:
            logger.error(
                f"SFTPService context exited with error: {exc_value}",
    ...
    def is_connected(self) -> bool:
        ...
        return (
            ...
            and self._ssh_client.get_transport().is_active()
            and self._sftp_client is not None
            and not self._sftp_client.sock.closed
        )
    ...
    def _ensure_connection(self) -> None:
        ...
        if not self.is_connected:
            logger.error(
                "SFTP client is not connected. Use within a 'with' block or"
    ...
    def _ensure_remote_dir_exists(self, remote_directory: str) -> bool:
        ...
        if not remote_directory or remote_directory == '.':
            logger.debug(
                "No remote directory creation needed for current/empty path."
            )
            return True
        ...
        if remote_directory.startswith('/'):
            ...
            current_parts = (
                []
            )  # Start from current remote directory (usually home)
        ...
        for dir_fragment in remote_directory.split('/'):
            ...
            try:
                ...
                logger.debug(
                    f"Remote directory fragment exists: {current_path}"
                )
            except IOError as e:
                # errno.ENOENT typically means "No such file or directory"
                if getattr(e, 'errno', None) == errno.ENOENT or isinstance(
                    e, FileNotFoundError
                ):
                    logger.info(
                        "Remote directory fragment not found, creating:"
                        f" {current_path}"
                    )
                    try:
                        sftp.mkdir(current_path)
                        logger.info(
                            "Successfully created remote directory:"
                            f" {current_path}"
                        )
                    except IOError as mkdir_e:
                        logger.error(
                            "Failed to create remote directory"
                            f" {current_path}: {mkdir_e}"
                        )
                    ...
                    logger.error(
                        f"Error checking remote directory {current_path}: {e}"
                    )
                    return False
            except Exception as e:
                logger.error(
                    "Unexpected error checking remote directory"
                    f" {current_path}: {e}",
                    exc_info=True,
                )
                return False
    ...
    def upload_file(self, local_path: Path, remote_path: str) -> None:
        ...
        remote_dir = os.path.dirname(
            remote_path
        )  # Use os.path for remote path manipulation
        ...
        if not self._ensure_remote_dir_exists(remote_dir):
            logger.error(
                f"Failed to ensure remote directory exists: {remote_dir}."
                " Aborting upload."
            )
            raise OSError(
                f"Could not create or access remote directory: {remote_dir}"
            )
        ...
        try:
            ...
            if (
                getattr(e, 'errno', None) == errno.EACCES
    ...
    def upload_files(self, file_mappings: dict[Path, str]) -> None:
        ...
        for local_path, remote_path in file_mappings.items():
            try:
                ...
                Exception
            ) as e:  # Catch any unexpected errors from upload_file
                logger.exception(
                    f"Unexpected error uploading {local_path} to"
                    f" {remote_path}: {e}"
                )
                fail_count += 1

        logger.info(
            f"Batch upload complete. Success: {success_count}, Failures:"
            f" {fail_count}"
        )
        if fail_count > 0:
            # Consider raising an exception here if any failure is critical
            logger.warning(
                "Some files failed to upload during batch operation."
            )
    ...
    def list_directory(self, remote_path: str = ".") -> List[str]:
        ...
        try:
            ...
            raise OSError(
                f"Failed to list directory {remote_path}: {e}"
            ) from e
    ...
    def change_directory(self, remote_path: str) -> None:
        ...
        try:
            ...
            logger.info(
                f"Remote directory changed to: {self._sftp_client.getcwd()}"
            )
        except (IOError, OSError, SSHException) as e:
            logger.error(
                f"Failed to change remote directory to {remote_path}: {e}"
            )
            raise OSError(
                f"Failed to change directory to {remote_path}: {e}"
            ) from e
    ...
    def get_current_directory(self) -> str:
        ...
        try:
            ...
            raise OSError(
                f"Failed to get current remote directory: {e}"
            ) from e
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "\nAttempting to connect and upload (will likely fail with dummy"
            " credentials)..."
        )
        try:
            ...
            with service:  # Connects on entry, closes on exit
                ...
                if service.is_connected:
                    ...
                    print(
                        "Connected! Current remote directory:"
                        f" {service.get_current_directory()}"
                    )
                    ...
                    remote_html_path = (
                        f"{remote_base_dir}/html/kb_report_20240914.html"
                    )
                    remote_png_path = (
                        f"{remote_base_dir}/thumbnails/thumbnail1.jpeg"
                    )

                    # Upload a single file
                    print(
                        f"\nUploading {report_html_path} to"
                        f" {remote_html_path}..."
                    )
                    ...
                    print(
                        f"\nUploading {report_thumbnail_path} to"
                        f" {remote_png_path}..."
                    )
                    ...
                    try:
                        ...
                        print(
                            "Could not list directory (may not exist or"
                    ...
                    print(
                        "Service failed to establish a connection within the"
                        " 'with' block."
                    )
