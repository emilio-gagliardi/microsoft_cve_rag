...
from typing import Optional, Tuple, Any, List, Dict, Union, AsyncGenerator # Added AsyncGenerator
import logging
import tiktoken
import asyncio
import time
...
MIN_LLM_CALL_INTERVAL_SECONDS = float(os.getenv("MIN_LLM_CALL_INTERVAL", "10.0"))
...
class LLMClient:
    ...
    def _count_tokens(text: str, model_name: str) -> int:
        ...
        elif "gpt-4" in normalized_model_name:  # Catches general "gpt-4" if not a more specific variant
    ...
    def get_completion(
        ...
        **kwargs: Any
        ...
        if self.mock_mode:
            ...
            return self._get_mock_completion(prompt + mock_image_info, system_prompt=system_prompt, model_name=model, task_type=task_type)[0]
            ...
            if isinstance(image_paths, str):
                ...
                for p in image_paths:
                    if isinstance(p, str):
                        ...
                        logger.warning(f"Unsupported image path type in list: {type(p)}. Skipping.")
            else:
                logger.warning(f"Unsupported image_paths type: {type(image_paths)}. Skipping image processing.")
        ...
        if image_path_obj_list: # Multi-modal message
            ...
            for img_path_obj in image_path_obj_list:
                ...
                if encoded_image_data:
                    ...
                    logger.warning(f"Could not encode image at {img_path_obj}. Skipping.")

            if not user_content_parts: # Should only happen if prompt was empty and all images failed
                logger.error("No content (prompt or image) to send to LLM for multi-modal call.")
                # _get_mock_completion returns (text, usage_dict)
                return self._get_mock_completion(prompt if prompt else "", system_prompt=system_prompt, model_name=model, task_type="error")[0]
            ...
            return self._get_mock_completion("", system_prompt=system_prompt, model_name=model, task_type="error")[0]
        ...
        if temperature is not None: completion_args["temperature"] = temperature
        if max_tokens is not None: completion_args["max_tokens"] = max_tokens
        if top_p is not None: completion_args["top_p"] = top_p
        if stream is not None: completion_args["stream"] = stream
        if stop is not None: completion_args["stop"] = stop
        if presence_penalty is not None: completion_args["presence_penalty"] = presence_penalty
        if frequency_penalty is not None: completion_args["frequency_penalty"] = frequency_penalty
        if response_format is not None: completion_args["response_format"] = response_format
        if seed is not None: completion_args["seed"] = seed
        if tools is not None: completion_args["tools"] = tools
        if tool_choice is not None: completion_args["tool_choice"] = tool_choice
        ...
        if logprobs:  # logprobs parameter from method signature is True
            ...
            if top_logprobs is not None: # top_logprobs from method signature (can be 0)
        ...
        if api_key is not None: completion_args["api_key"] = api_key
        if api_base is not None: completion_args["api_base"] = api_base
        if api_version is not None: completion_args["api_version"] = api_version
        if deployment_id is not None: completion_args["deployment_id"] = deployment_id
        if metadata is not None: completion_args["metadata"] = metadata

        # Remove logprobs and top_logprobs from kwargs before update, as they are handled explicitly from method params
        kwargs.pop('logprobs', None)
        kwargs.pop('top_logprobs', None)
        kwargs.pop('task_type', None) # Ensure task_type is not passed to LiteLLM

        # Add any other kwargs passed directly
        completion_args.update(kwargs)

        logger.debug(f"Calling litellm.completion with args: {completion_args}")
        try:
            ...
            if usage:
                logger.info(
                    f"LiteLLM call to {model} successful. Usage: Prompt tokens: {usage.prompt_tokens}, "
                    f"Completion tokens: {usage.completion_tokens}, Total tokens: {usage.total_tokens}"
                )
            else:
                logger.info(f"Received completion from {model}. Length: {len(completion_text)}. Usage data not available.")
    ...
    async def aget_completion(
        ...
        **kwargs: Any
        ...
        if self.mock_mode:
            mock_image_info = f" (with {len(image_paths)} images)" if image_paths else ""
            task_type = kwargs.get('task_type', 'default')
            logger.info(f"\n\nLLM Client (MOCK) received prompts for task '{task_type}':\nsystem prompt:\n{system_prompt}\nuser prompt:\n{prompt}{mock_image_info}\n\n")
            # _get_mock_completion now returns (text, usage_dict)
            return self._get_mock_completion(prompt + mock_image_info, system_prompt=system_prompt, model_name=model, task_type=task_type)
        logger.info(f"Type of 'prompt' variable: {type(prompt)}, Value: {prompt}")
        messages = []
        if system_prompt:
            messages.append({"role": "system", "content": system_prompt})
        ...
        if image_path_obj_list: # Multi-modal message
            ...
            if not user_content_parts: # Should only happen if prompt was empty and all images failed
                logger.error("No content (prompt or image) to send to LLM for multi-modal call.")
                return self._get_mock_completion(prompt if prompt else "", system_prompt=system_prompt, model_name=model, task_type="error")
            messages.append({"role": "user", "content": user_content_parts})
        elif prompt: # Text-only message
            messages.append({"role": "user", "content": prompt})
        else: # No prompt and no images
            logger.error("No prompt provided and no images to send.")
            return self._get_mock_completion("", system_prompt=system_prompt, model_name=model, task_type="error")

        # Dynamically build the arguments dictionary for litellm.acompletion
        kwargs_for_acompletion = {}

        if temperature is not None: kwargs_for_acompletion["temperature"] = temperature
        if max_tokens is not None: kwargs_for_acompletion["max_tokens"] = max_tokens
        if top_p is not None: kwargs_for_acompletion["top_p"] = top_p
        if stream is not None: kwargs_for_acompletion["stream"] = stream # stream can be False
        if stop is not None: kwargs_for_acompletion["stop"] = stop
        if presence_penalty is not None: kwargs_for_acompletion["presence_penalty"] = presence_penalty
        if frequency_penalty is not None: kwargs_for_acompletion["frequency_penalty"] = frequency_penalty
        if response_format is not None: kwargs_for_acompletion["response_format"] = response_format
        if seed is not None: kwargs_for_acompletion["seed"] = seed
        if tools is not None: kwargs_for_acompletion["tools"] = tools
        if tool_choice is not None: kwargs_for_acompletion["tool_choice"] = tool_choice
        ...
        if api_key is not None: kwargs_for_acompletion["api_key"] = api_key
        if api_base is not None: kwargs_for_acompletion["api_base"] = api_base
        if api_version is not None: kwargs_for_acompletion["api_version"] = api_version
        if deployment_id is not None: kwargs_for_acompletion["deployment_id"] = deployment_id
        if timeout is not None: kwargs_for_acompletion["timeout"] = timeout
        if metadata is not None: kwargs_for_acompletion["metadata"] = metadata
        ...
        async with _llm_call_lock:
            ...
            if time_since_last_call < MIN_LLM_CALL_INTERVAL_SECONDS:
                delay_needed = MIN_LLM_CALL_INTERVAL_SECONDS - time_since_last_call
                logger.info(f"Throttling LLM call. Waiting for {delay_needed:.2f} seconds.")
                await asyncio.sleep(delay_needed)

            logger.debug(f"Calling litellm.acompletion with model '{model}' and messages: {messages}. Full args: {kwargs_for_acompletion}")
            try:
                response_obj: Union[ModelResponse, AsyncGenerator[ModelResponse, None]] = await litellm.acompletion(
                    model=model,
                    messages=messages,
                    **kwargs_for_acompletion
                )
                ...
                if isinstance(response_obj, ModelResponse): # Non-streaming
                    ...
                    if response_obj.usage:
                        ...
                        logger.info(
                            f"LiteLLM async call to {model} successful. Usage: Prompt tokens: {usage_dict['prompt_tokens']}, "
                            f"Completion tokens: {usage_dict['completion_tokens']}, Total tokens: {usage_dict['total_tokens']}"
                        )
                    else:
                        logger.info(f"Received async completion from {model}. Length: {len(completion_text)}. Usage data not available.")
                elif isinstance(response_obj, AsyncGenerator): # Streaming
                    logger.info(f"Receiving streamed async completion from {model}...")
                    ...
                    async for chunk in response_obj:
                        ...
                        if hasattr(chunk, 'usage') and chunk.usage: # Check if usage is present and not None
                    ...
                    logger.info(f"Finished receiving streamed async completion from {model}. Total length: {len(completion_text)}.")
                else:
                    logger.warning(f"Unexpected response_obj type from litellm.acompletion: {type(response_obj)}")
    ...
    def _get_mock_completion(
        ...
        logger.debug(f"Generating mock completion for task_type: {task_type} with model: {model_name}")
        ...
        mock_input_tokens = 100 + (50 if system_prompt else 0)  # Rough estimate for prompt + system
        mock_completion_tokens = len(mock_text.split()) // 2  # Very rough estimate for completion
    ...
    try:
        ...
        try:
            ...
            if mime_type is None:
                mime_type = "image/png" # Default if guess fails, e.g. for .webp or other types not in standard mimetypes

            image_data_url = f"data:{mime_type};base64,{base64_image}"

            messages_payload = [
                {'role': 'system', 'content': 'You are a data-focused security analyst.'},
                {'role': 'user', 'content': [
        ...
        print(f"\n--- Attempting direct call to litellm.acompletion with model: {model_to_test} ---")
        ...
        print(f"LiteLLM BadRequestError (Direct Call for {model_to_test}): {e}")
        ...
        elif hasattr(e, 'message'): # Fallback to the direct message if response object is not as expected
             error_response_text = str(e.message)
        print(f"Error details: {error_response_text}")

    except Exception as e:
        print(f"An unexpected error occurred (Direct Call for {model_to_test}): {e}")
        import traceback
        traceback.print_exc()
