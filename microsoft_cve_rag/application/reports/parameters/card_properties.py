...
STAT_CARD_SUBTITLES = {
    ...
    "cve_category_distribution": (
        "Various metrics of CVEs by vulnerability category"
    ),
    ...
    "worst_case_cves": (
        "Weighted score (network + low complexity + low privileges + no"
        " interaction)"
    ),
    # 6. CWE Weaknesses
    ...
    "spotlight_cwe_CWE-16": (
        "System Configuration: Weakness, leads to vulnerabilities from"
        " insecure ..."
    ),
    "spotlight_cwe_CWE-20": (
        "Input: Improperly validated, leads to security vulnerabilities ..."
    ),
    "spotlight_cwe_CWE-74": (
        "Downstream Output Special Elements: Not neutralized, leads to"
        " injection ..."
    ),
    "spotlight_cwe_CWE-78": (
        "OS Command Special Elements: Not neutralized, leads to OS command ..."
    ),
    "spotlight_cwe_CWE-79": (
        "Web Page Generation Input: Not neutralized, leads to cross-site ..."
    ),
    "spotlight_cwe_CWE-89": (
        "SQL Command Special Elements: Not neutralized, leads to SQL"
        " injection ..."
    ),
    "spotlight_cwe_CWE-94": (
        "Code Generation: Improperly controlled, leads to code injection ..."
    ),
    "spotlight_cwe_CWE-119": (
        "Memory Buffer Operations: Improperly restricted within defined"
        " bounds ..."
    ),
    "spotlight_cwe_CWE-120": (
        "Buffer Copy Input Size: Not checked ('Classic Buffer Overflow'),"
        " leads ..."
    ),
    "spotlight_cwe_CWE-121": (
        "Stack-Based Buffer: Overflow, leads to crashes or arbitrary code ..."
    ),
    "spotlight_cwe_CWE-122": (
        "Heap-Based Buffer: Overflow, leads to crashes or arbitrary code ..."
    ),
    "spotlight_cwe_CWE-124": (
        "Buffer Underwrite: Data written before buffer start ('Buffer"
        " Underflow') ..."
    ),
    "spotlight_cwe_CWE-125": (
        "Out-of-Bounds Read: Occurs outside allocated memory, leads to"
        " info ..."
    ),
    "spotlight_cwe_CWE-188": (
        "Data/Memory Layout of Type: Reliance upon, leads to portability"
        " or ..."
    ),
    "spotlight_cwe_CWE-276": (
        "Default Permissions: Incorrectly set, leads to unauthorized"
        " access ..."
    ),
    "spotlight_cwe_CWE-284": (
        "Access Control: Improperly implemented, leads to unauthorized"
        " actions ..."
    ),
    "spotlight_cwe_CWE-285": (
        "Authorization: Improperly performed, leads to privilege"
        " escalation ..."
    ),
    "spotlight_cwe_CWE-310": (
        "Cryptographic Functions/Issues: Flawed use or implementation, leads"
        " to ..."
    ),
    "spotlight_cwe_CWE-326": (
        "Encryption Strength: Inadequate (weak algorithm/key), leads to"
        " data ..."
    ),
    "spotlight_cwe_CWE-337": (
        "PRNG Seed: Predictable, leads to compromised random values for ..."
    ),
    "spotlight_cwe_CWE-338": (
        "Weak PRNG: Used in security context, leads to predictable values ..."
    ),
    "spotlight_cwe_CWE-362": (
        "Shared Resource (Concurrent): Improperly synchronized ('Race"
        " Condition') ..."
    ),
    "spotlight_cwe_CWE-416": (
        "Use After Free: Referencing memory after deallocation, leads to ..."
    ),
    "spotlight_cwe_CWE-444": (
        "HTTP Request Interpretation: Inconsistent ('HTTP Request"
        " Smuggling') ..."
    ),
    "spotlight_cwe_CWE-610": (
        "Cross-Sphere Resource Reference: Externally controlled, leads to ..."
    ),
    "spotlight_cwe_CWE-632": (
        "User Interface Security: Weaknesses, leads to client-side attacks ..."
    ),
    "spotlight_cwe_CWE-639": (
        "Authorization Bypass (User Key): Using user-controlled key, leads"
        " to ..."
    ),
    "spotlight_cwe_CWE-667": (
        "Resource Locking: Improperly performed, leads to race conditions"
        " or ..."
    ),
    "spotlight_cwe_CWE-703": (
        "Exceptional Conditions: Improperly checked or handled, leads to ..."
    ),
    "spotlight_cwe_CWE-707": (
        "Downstream Output Special Elements: Not neutralized, leads to"
        " injection ..."
    ),
    "spotlight_cwe_CWE-732": (
        "Critical Resource Permissions: Incorrectly assigned, leads to ..."
    ),
    "spotlight_cwe_CWE-762": (
        "Memory Management Routines: Mismatched for alloc/dealloc, leads"
        " to ..."
    ),
    "spotlight_cwe_CWE-783": (
        "Operator Precedence Logic: Error in evaluation order, leads to ..."
    ),
    "spotlight_cwe_CWE-787": (
        "Out-of-Bounds Write: Data written outside allocated memory, leads"
        " to ..."
    ),
    "spotlight_cwe_CWE-840": (
        "Business Logic: Errors in design or implementation, leads to ..."
    ),
    "spotlight_cwe_CWE-841": (
        "Behavioral Workflow: Improperly enforced by application, leads to ..."
    ),
    "spotlight_cwe_CWE-642": (
        "Critical State Data: Externally controlled, leads to manipulation ..."
    ),
    # 7. Patch Timeliness
...
CARD_STYLES = {
    ...
    "critical_count_card": {
        "container": (  # Neutral card, danger border
            "bg-status-danger-bg dark:bg-status-danger-bg-dark border"
            " border-status-danger dark:border-status-danger-border"
        ),
        # Title Bar themed for danger
        "title_bar_bg": "",
        "title_bar_padding": "py-1 px-1",
        "label_text_style": f"{BASE_TEXT_SIZES['label']}",
        "label_text_color": (  # Text color for danger bar
            "text-status-danger-text dark:text-status-danger-text-dark"
        ),
        "icon_style": "w-4 h-4 opacity-75",
        "icon_color": (  # Icon color for danger bar
            "text-status-danger-text dark:text-status-danger-text-dark"
        ),
        # Content Area
        "content_area_bg": "",
        # Value, Unit, Subtitle, Trend styles
        "value": (  # Value text is danger-themed
            f"{BASE_TEXT_SIZES['value']} text-status-danger-text"
            " dark:text-status-danger-text-dark"
        ),
        "unit": (  # Unit also danger-themed
            f"{BASE_TEXT_SIZES['unit']} text-status-danger/80"
            " dark:text-status-danger-text-dark/80"
        ),
        "subtitle": (  # Subtitle can be neutral
            f"{BASE_TEXT_SIZES['subtitle']} text-status-danger-text"
            " dark:text-status-danger-text-dark"
        ),
        "subtitle_bar_padding": "px-2 pb-2 pt-1",
        "trend_base": f"{BASE_TEXT_SIZES['trend']}",
    ...
    "critical_high_pct_card": {
        "container": (  # Neutral card, danger border
            "bg-status-warning-bg dark:bg-status-warning-bg-dark border"
            " border-status-warning dark:border-status-warning-border"
        ),
        # Title Bar themed for danger
        "title_bar_bg": "",
        "title_bar_padding": "py-1 px-1",
        ...
        # Value, Unit, Subtitle, Trend styles
        "value": (  # Value text is danger-themed
            f"{BASE_TEXT_SIZES['value']} text-status-warning-text"
            " dark:text-status-warning-text-dark"
        ),
        "unit": (  # Unit also danger-themed
            f"{BASE_TEXT_SIZES['unit']} text-status-warning-text"
            " dark:text-status-warning-text-dark"
        ),
        "subtitle": (  # Subtitle can be neutral
            f"{BASE_TEXT_SIZES['subtitle']} text-status-warning-text"
            " dark:text-status-warning-text-dark"
        ),
        "subtitle_bar_padding": "px-2 pb-2 pt-1",
        "trend_base": f"{BASE_TEXT_SIZES['trend']}",
    ...
    "cvss_iqr_by_category_style": {
        "container": (
            ...
            " border-report-border-light dark:border-report-border-dark"
            " text-center"
        ),
        "label": (
            f"{BASE_TEXT_SIZES['label']} text-report-text-muted-light"
            " dark:text-report-text-muted-dark"
        ),
        "icon": "text-brand-blue dark:text-brand-blue-light opacity-75",
        "value": (
            f"{BASE_TEXT_SIZES['value']} text-report-text-light"
            " dark:text-report-text-dark"
        ),
        "unit": (
            f"{BASE_TEXT_SIZES['value']} text-report-text-light"
            " dark:text-report-text-dark opacity-80 pl-0.5"
        ),
        "subtitle": (
            f"{BASE_TEXT_SIZES['subtitle']} text-report-text-muted-light"
            " dark:text-report-text-muted-dark  text-xxs leading-[1.1]"
        ),
        "subtitle_bar_padding": "px-2 pb-2 pt-1",
        "trend_base": f"{BASE_TEXT_SIZES['trend']}",
        "trend_up": "",
    ...
    "top_impact_type_card": {
        ...
        # Value, Unit, Subtitle, Trend styles
        "value": (
            "text-1xl md:text-1xl font-bold text-report-text"
            " dark:text-report-text-dark leading-[1.1]"
        ),
        "unit": (
            f"{BASE_TEXT_SIZES['unit']} text-report-text-muted"
            " dark:text-report-text-muted-dark"
    ...
    "spotlight_cwe_CWE-16_card": {
        ...
        "value_container": (
            "my-auto text-center"
        ),  # For the div wrapping value and its subtexts
        "value": (  # Added block & leading-tight
            f"{BASE_TEXT_SIZES['value']} text-report-text"
            " dark:text-report-text-dark block leading-[1.1]"
        ),
        "predominant_category_text": (
            f"{BASE_TEXT_SIZES.get('cwe_extra_info', 'text-xs')} font-semibold"
            " mt-1 leading-[1.1]"
        ),
        "avg_cvss_text": (
            f"{BASE_TEXT_SIZES.get('cwe_extra_info', 'text-xs')} text-report-text-muted"
            " dark:text-report-text-muted-dark mt-0.5 leading-[1.1]"
        ),
        "subtitle": (
...
CVE_CATEGORY_COLOR_MAP = {
    ...
    'default': (  # Fallback for unmapped keys
        'bg-chart-gray-2 dark:bg-chart-gray-2-dark'
    ),
}
